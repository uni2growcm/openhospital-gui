...
public class PatientBillEdit extends JDialog implements SelectionListener {
			...
			if (GeneralData.STOCKMVTONBILLSAVE) {
				if (selectedBillItem.getId() > 0) {
					BillItems originalItem = originalBillItems.stream().filter(
							bi -> bi.getId() == selectedBillItem.getId()).findFirst().orElse(null);
	...
	private MedicalWard getMedicalWard(Price price) {
		if (GeneralData.STOCKMVTONBILLSAVE && (price == null || medWardList == null || medWardList.isEmpty())) {
			JOptionPane.showMessageDialog(
					PatientBillEdit.this,
	...
	private Price getPrice(MedicalWard med) {
		...
		for (Price price : prcListArray) {
			if (price != null && "MED".equals(price.getGroup()) && Objects.equals(price.getItem(), String.valueOf(med.getMedical().getCode()))) {
				return price;
			}
		}
		return null;
		...
		if (price != null) {
			...
			if (GeneralData.STOCKMVTONBILLSAVE) {
				if (containPrice(price, qty)) {
					billItem = addItem(price, qty, true);
				} else {
					JOptionPane.showMessageDialog(PatientBillEdit.this,
							MessageBundle.getMessage("angal.newbill.qtynotinstock"),
	...
	private BillItems addOtherPrice(Price oth, int qty) {
		...
		if (qty <= 0) qty = 1;
		List<PricesOthers> othPrices = null;
		PricesOthers pricesOther = null;
		if (oth != null) {
			try {
				othPrices = pricesOthersManager.getOthers();
			...
			if (othPrices != null) {
				Price finalOth = oth;
				pricesOther = othPrices.stream()
						.filter(p -> p.getId() == Integer.parseInt(finalOth.getItem()))
						.findFirst()
						.orElse(null);
			}
			if (pricesOther != null && pricesOther.isUndefined()) {
				Icon icon = new ImageIcon("rsc/icons/money_dialog.png");
				String price = (String) JOptionPane.showInputDialog(PatientBillEdit.this,
						MessageBundle.getMessage("angal.newbill.howmuchisit"),
						MessageBundle.getMessage("angal.newbill.undefined"),
						JOptionPane.PLAIN_MESSAGE, icon, null, "0");
				try {
					if (price == null) return null;
			...
			if (pricesOther != null && pricesOther.isDischarge()) {
				double amount = oth.getPrice();
				oth.setPrice(-amount);
			}
			item = addItem(oth, qty, isPrice);
		}
		return item;
	}
		...
		if (jButtonAddMedical == null) {
			...
			jButtonAddMedical.addActionListener(actionEvent -> {
				...
				Price med = (Price) JOptionPane.showInputDialog(this, MessageBundle.getMessage("angal.newbill.selectamedical.txt"),
						MessageBundle.getMessage("angal.newbill.medical.title"), JOptionPane.PLAIN_MESSAGE, icon, medArray.toArray(), "");
				if (med != null) {
					int qty = 1;
					String quantity = (String) JOptionPane.showInputDialog(this, MessageBundle.getMessage("angal.newbill.insertquantity.txt"),
		...
		if (jButtonAddOperation == null) {
			...
			jButtonAddOperation.addActionListener(actionEvent -> {
				...
				Price ope = (Price) JOptionPane.showInputDialog(this, MessageBundle.getMessage("angal.newbill.selectanoperation.txt"),
						MessageBundle.getMessage("angal.newbill.operation.title"), JOptionPane.PLAIN_MESSAGE, icon, opeArray.toArray(), "");
				addItem(ope, 1, true);
			});
		}
		...
		if (jButtonAddExam == null) {
			...
			jButtonAddExam.addActionListener(actionEvent -> {
				...
				Price exa = (Price) JOptionPane.showInputDialog(this, MessageBundle.getMessage("angal.newbill.selectanexam.txt"),
						MessageBundle.getMessage("angal.newbill.exam.title"), JOptionPane.PLAIN_MESSAGE, icon, exaArray.toArray(), "");
				addItem(exa, 1, true);
			});
		}
	...
	private JButton getJButtonAddOther() {
		if (jButtonAddOther == null) {
			...
			jButtonAddOther.addActionListener(actionEvent -> {
				List<Price> otherArray = new ArrayList<>();
				for (Price price : prcListArray) {
					if (price.getGroup().equals("OTH")) {
						otherArray.add(price);
					}
				}
				...
				Price oth = (Price) JOptionPane.showInputDialog(this, MessageBundle.getMessage("angal.newbill.pleaseselectanitem.txt"),
						MessageBundle.getMessage("angal.newbill.item.title"), JOptionPane.PLAIN_MESSAGE, icon, otherArray.toArray(), "");
				if (oth != null) {
					List<PricesOthers> othPricesList = null;
					PricesOthers pricesOther = null;
					try {
						othPricesList = pricesOthersManager.getOthers();
					} catch (OHServiceException e) {
						throw new RuntimeException(e);
					}
					if (othPricesList != null) {
						Price finalOth = oth;
						pricesOther = othPricesList.stream()
								.filter(p -> p.getId() == Integer.parseInt(finalOth.getItem()))
								.findFirst()
								.orElse(null);
					}
					if (pricesOther != null && pricesOther.isUndefined()) {
						...
						String price = (String) JOptionPane.showInputDialog(this, MessageBundle.getMessage("angal.newbill.howmuchisit.txt"),
								MessageBundle.getMessage("angal.common.undefined.txt"), JOptionPane.PLAIN_MESSAGE, icon, null, "0");
						try {
							if (price == null) {
								return;
							}
					...
					if (pricesOther != null && pricesOther.isDaily()) {
						int qty = 1;
	...
	private JButton getJButtonAddCustom() {
		if (jButtonCustom == null) {
			...
			jButtonCustom.addActionListener(actionEvent -> {
				double amount;
				Icon icon = new ImageIcon("rsc/icons/custom_dialog.png");
				...
				try {
					BillItems newItem = new BillItems(0, billBrowserManager.getBill(thisBill.getId()), false, "",
							desc, amount, 1);
					addItem(newItem);
				} catch (OHServiceException ohServiceException) {
					MessageDialog.showExceptions(ohServiceException);
			...
			jButtonAddPayment.addActionListener(actionEvent -> {
				...
				if (balance.compareTo(BigDecimal.ZERO) != 0) {
					if (balance.equals(bigTotal)) {
						...
						amount = balance;
						MessageDialog.inputDialog(this, icon, null, amount, "angal.newbill.insertquantity.txt");
					}
				} else {
					return;
				}
				if (thisBill.getDate().isBefore(today)) {
					...
					LocalDateTime datePay = TimeTools.getNow();
					addPayment(datePay, amount.doubleValue());
				}
			});
		}
	...
	private JButton getJButtonPrintBill() {
		if (jButtonPrintBill == null) {
			...
			jButtonPrintBill.addActionListener(actionEvent -> {
				...
				if (thisBill.getId() > 0) {
					new GenericReportBill(thisBill.getId(), GeneralData.PATIENTBILL);
				} else if (insert && thisBill.getBillPatient() != null) {
					int saveOption = MessageDialog.yesNo(PatientBillEdit.this, "angal.billbrowser.billsavenow.msg");
					if (saveOption == JOptionPane.YES_OPTION) {
						jButtonSave.doClick();
						new GenericReportBill(thisBill.getId(), GeneralData.PATIENTBILL);
					}
				} else {
					MessageDialog.error(PatientBillEdit.this, "angal.billbrowser.invoiceerror.msg");
				}
			});
		}
		setJButtonPrintBill();
	...
	private void setJButtonPrintBill() {
		if (jButtonPrintBill != null && insert) {
			...
			jButtonSave.addActionListener(actionEvent -> {
				...
				if (insert) {
					Bill newBill = new Bill(
							0, thisBill.getDate(), null, true, thisBill.getPriceList(),
							thisBill.getPriceList().getName(), thisBill.isPatient(),
							thisBill.getBillPatient(), thisBill.isPatient() ? thisBill.getBillPatient().getName() : jTextFieldPatient.getText(),
					...
					Bill updateBill = new Bill(
							thisBill.getId(), thisBill.getDate(), null, true, thisBill.getPriceList(),
							thisBill.getPriceList().getName(), thisBill.isPatient(),
							thisBill.getBillPatient(), thisBill.isPatient() ? thisBill.getPatName() : jTextFieldPatient.getText(),
							paid ? "C" : "O", total.doubleValue(), balance.doubleValue(), user, thisBill.getAdmission());
					if (hasBillGuarantor()) {
						User guarantor = (User) jComboBoxGuarantor.getSelectedItem();
						if (balance.doubleValue() != 0) {
				...
				if (balance.compareTo(new BigDecimal(0)) > 0) {
					if (thisBill.getDate().isBefore(today)) {
						...
						int r = JOptionPane.showConfirmDialog(this, datePayChooser,
								MessageBundle.getMessage("angal.newbill.dateofpayment.title"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, icon);
						if (r == JOptionPane.OK_OPTION) {
							LocalDateTime datePay = datePayChooser.getLocalDateTime();
							if (isValidPaymentDate(datePay)) {
						...
						addPayment(TimeTools.getNow(), balance.doubleValue());
					}
				}
				paid = true;
				updateBalance();
	...
	public class BillTableModel implements TableModel {
		...
		public Class<?> getColumnClass(int columnIndex) {
			return billClasses[columnIndex].getClass();
		}
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			BillItems item = billItems.get(rowIndex);
			if (columnIndex == -1) return item;
			if (columnIndex == 0) return item.getItemDescription();
			if (columnIndex == 1) return item.getItemQuantity();
			if (columnIndex == 2) {
	...
	public class PaymentTableModel implements TableModel {
		...
		@Override
		public Object getValueAt(int rowIndex, int columnIndex) {
			if (columnIndex == -1) return payItems.get(rowIndex);
			if (columnIndex == 0) return formatDateTime(payItems.get(rowIndex).getDate());
			if (columnIndex == 1) return payItems.get(rowIndex).getAmount();
			return null;
		}

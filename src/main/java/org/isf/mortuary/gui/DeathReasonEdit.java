	...
	private JButton getSaveButton() {
		...
		saveButton.addActionListener(actionEvent -> {
			if (insert) {
				...
				if (code.length() > 11) {
					MessageDialog.error(this, "angal.mortuary.deathreason.codeexceedsmaxlengthofelevenchars.msg");
					return;
				}
				try {
					if (deathReasonManager.isCodePresent(code)) {
			...
			deathReason.setDescription(descriptionTextField.getText().trim());
			deathReason.setCode(codeTextField.getText().trim());
